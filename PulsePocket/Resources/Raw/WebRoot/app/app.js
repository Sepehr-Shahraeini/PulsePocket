...
app.config(function ($routeProvider) {
    ...
    $routeProvider.when("/appcertificate/last", {
        controller: "appCertificateController",
        templateUrl: "app/views/appCertificate.html",
        type: 'last',
    });
...
app.run(['authService', 'activityService', '$rootScope', '$location', '$window', '$templateCache', function (authService, activityService, $rootScope, $location, $window, $templateCache) {
    ...
    window.addEventListener("message", function (event) {
        $rootScope.$apply(function () {
            ...
            $rootScope.userName = event.data.userName;
            $rootScope.userTitle = event.data.userName;
            $rootScope.userId = event.data.userId;
            $rootScope.employeeId = event.data.employeeId;
            $rootScope.jobGroup = event.data.Position;
            $rootScope.JobGroup2 = event.data.Position;

            if (event.data.TIndex == "1") {
                $location.path("/appflight");
                $rootScope.$broadcast("ReloadFlights", null);
            }
            else if (event.data.TIndex == "4") {
                $location.path("/forms");
                $rootScope.$broadcast("ReloadForms", null);
            }
            else if (event.data.TIndex == "3") {
                $location.path("/appcertificate/last");
                $rootScope.$broadcast("ReloadCertificate", null);
            }
        });
    });
    ...
    $rootScope.navigate = function (target) {
    ...
    function GetPageIndex() {
        HybridWebView.SendInvokeMessageToDotNet("GetPageIndex");
    }
